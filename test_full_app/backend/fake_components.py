...
class DeterministicFakeLLM:
    ...
    def _analyze_prompt(self, prompt: str, context: str = "") -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "is_follow_up": any(word in prompt_lower for word in ["also", "additionally", "furthermore", "what about"])
        }
        ...
        for i, word in enumerate(words):
            if word.upper().startswith("PN-") or "part" in word.lower():
                ...
                if i < len(words) - 1:
                    analysis["entities"].append({"type": "issue", "value": words[i+1]})
...
class FakeVectorRetriever:
    ...
    def __init__(self, documents: Optional[List[Document]] = None):
        ...
        for i, doc in enumerate(self.documents):
            self.document_embeddings[i] = self.embeddings.embed_documents([doc.page_content])[0]
    ...
    def _create_default_documents(self) -> List[Document]:
        ...
        return [
            Document(
                ...
                metadata={"filename": "troubleshooting_guide.pdf", "part_number": "PN-001", "section": "error_handling"}
            ),
            Document(
                ...
                metadata={"filename": "maintenance_manual.pdf", "part_number": "PN-002", "section": "routine_maintenance"}
            ),
            Document(
                ...
                metadata={"filename": "parts_catalog.pdf", "part_number": "PN-003", "section": "components"}
            ),
            Document(
                ...
                metadata={"filename": "diagnostic_manual.pdf", "part_number": "PN-004", "section": "diagnostics"}
            ),
            Document(
                ...
                metadata={"filename": "operation_manual.pdf", "part_number": "PN-005", "section": "procedures"}
            )
        ]
    ...
    def _get_relevant_documents(self, query: str, **kwargs) -> List[Document]:
        ...
        return top_docs if top_docs else [self.documents[0]]  # Always return at least one document
...
def create_test_retriever(documents: Optional[List[Document]] = None) -> FakeVectorRetriever:
