...
2. **Starting the Simulator** – `poetry run uvicorn test_full_app.backend.simulator:app` is spawned in `drsearch_backend` with the chosen port. Output is logged to `test_full_app/logs/simulator1.*.log`.
...
6. **Running Playwright Tests** – `npx playwright test test_full_app/frontend` executes all specs. When `COLLECT_COVERAGE=1` is present, the environment variable `PW_TEST_COVERAGE=1` is forwarded so coverage data is emitted.
...
`test_full_app/backend/simulator.py` serves two endpoints used by the tests:
