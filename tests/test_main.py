...
def test_main_runs(monkeypatch):
    ...
    monkeypatch.setattr("app.main.get_settings", lambda: types.SimpleNamespace(debug=False, auth_enabled=True))

    # Patch create_app to return a dummy ASGI app
    monkeypatch.setattr("app.main.create_app", lambda: lambda scope, receive, send: None)
    ...
    class DummyLoop:
        def add_signal_handler(self, *_, **__): pass
